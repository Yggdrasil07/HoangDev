...
exports.handleEvent = async function(o) {
  try {
   /* var {
    ...
    const res = (await axios.get(`https://j2download.net/api/facebook/media?url=${encodeURIComponent(url)}`)).data;
    let attachment = [];
    ...
    let attachment = [];
     if (res.attachments && res.attachments.length > 0) {
      ...
      /* TỰ ĐỘNG TẢI NHẠC TRÊN SOUNDCLOUD */
        ...
        const res = await axios.get(`http://nguyenmanh.name.vn/api/scDL?url=${str}&apikey=yYmZAYqn`);
        send({
          body: `📝 Title: ${res.data.result.title}\n✏️ MôMô Tả: ${res.data.result.data.description}\n👁️‍🗨️ Lượt Xem: ${res.data.result.data.playback_count}`,
      ...
      /* TỰ ĐỘNG TẢI NHẠC ZINGMP3 */
        ...
        const tz = await axios.get(`https://haohoang.me/api/zingmp3/info?link=${str}`);
        send({
      ...
      /* TỰ ĐOONGJ TẢI NHẠC SPOTIFY */
        ...
        const res = await axios.get(`https://test-ai-ihc6.onrender.com/mid/spotify-v2?url=${str}`);
        send({
